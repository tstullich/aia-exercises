...
Mat Aia1::doSomethingThatMyTutorIsGonnaLike(Mat& img){
    Mat outputImg = Mat(img.size(), CV_8UC3); ;
    int row,col;
    //Compute the “negative” of a RGB image
    for (row = 0; row < img.size().height; row++) {
        for (col = 0; col < img.size().width; col++) {
            cv::Vec3b sourceVector = img.at<cv::Vec3b>(row,col);
            cv::Vec3b inverseVector = cv::Vec3b(255 - sourceVector[0], 255 - sourceVector[1], 255 - sourceVector[2]);
            outputImg.at<cv::Vec3b>(row, col) = inverseVector;
...
void Aia1::test_doSomethingThatMyTutorIsGonnaLike(Mat& inputImage, Mat& outputImage){
    ...
    long numOfPlanes = inputPlanes.size();
