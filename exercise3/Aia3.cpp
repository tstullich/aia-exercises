...
void Aia3::plotHough(vector< vector<Mat> >& houghSpace){
    ...
    // Project down angle- and scale-dimensions to a single image
    Mat image = Mat::zeros(houghSpace[0][0].rows, houghSpace[0][0].cols, CV_32FC1);
    for(int i = 0; i < houghSpace.size(); i++){
        for (int j = 0; j < houghSpace[i].size(); j++) {
            add(image, houghSpace[i][j], image);
        }
    }

    // Display image
    showImage(image, "Hough Space", 0); 

    // Write image to file  
    imwrite("houghSpace.png", image);
}
